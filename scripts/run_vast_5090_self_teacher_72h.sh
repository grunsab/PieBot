...
# Restart-safe 72-hour PieBot self-teacher run for the 46-vCPU Vast.ai VM.
# A stable output root preserves autopilot's absolute deadline and partial stages.
...
INITIAL_CHECKPOINT_SOURCE="${INITIAL_CHECKPOINT_SOURCE:-/workspace/piebot_runs/main_48h_20260802T081500Z/cycles/cycle_000086/train/checkpoint.json}"
...
INITIAL_ACTIVE_MODEL_SOURCE="${INITIAL_ACTIVE_MODEL_SOURCE:-/workspace/piebot_runs/main_48h_20260802T081500Z/cycles/cycle_000083/nnue_quant.nnue}"
...
SELFPLAY_PARALLEL_GAMES="${SELFPLAY_PARALLEL_GAMES:-46}"
RELABEL_DEPTH="${RELABEL_DEPTH:-5}"
...
[[ "$VALIDATION_PROVENANCE_SHA256" =~ ^[0-9a-fA-F]{64}$ ]] \
  || die "VALIDATION_PROVENANCE_SHA256 must contain exactly 64 hexadecimal characters"
[[ "$OUT_ROOT" != "/workspace/piebot_runs/main_48h_20260802T081500Z" ]] \
  || die "OUT_ROOT must not overwrite the previous training run"
...
available = shutil.disk_usage(Path("/workspace/piebot_runs")).free
...
log "training deadline reached cleanly; state: $OUT_ROOT/autopilot_state.json"
