#!/usr/bin/env python3
"""Safely migrate PieBot LCZero training pipeline to JSON checkpoint optimization and caching.
...
5. Checkout of target branch campaign/lc0-parallel-ingestion (commit 6dca84b) in /workspace/piebot_lc0_repo.
...
OPS_DIR = Path("/workspace/piebot_ops/checkpoint_optimization_20260916")
...
TARGET_COMMIT = "6dca84bfe07ba41bb7176e5ab8dd2fc024aa2a91"
...
def stop_piebot() -> dict:
    ...
    if "STOPPED" not in status_res.stdout:
        log("Requesting clean stop of piebot_lc0 at chunk boundary (waiting up to 400s)...")
        res = run_cmd(["supervisorctl", "stop", "piebot_lc0"], timeout=450.0)
...
def restart_and_verify(old_chunks: int, target_commit: str, source_id: dict) -> dict:
    log("Starting piebot_lc0 with checkpoint JSON optimization...")
    ...
    receipt = {
        "transition": "piebot-lc0-checkpoint-optimization-v1",
