...
#[command(
    name = "piebot-bench",
    version,
    about = "Benchmark PieBot search NPS with PST/NNUE"
)]
struct Args {
...
fn run_pleco(
    ...
    let mut board = if fen == "startpos" {
        ...
        pleco::Board::from_fen(fen).expect("valid FEN")
    };
...
fn main() {
    ...
    let board = if args.fen == "startpos" {
        ...
        Board::from_fen(&args.fen, false).expect("valid FEN")
    };
    ...
    let eval_mode = if args.material_only {
        EvalMode::Material
    } else if let Some(mode) = args.eval.as_deref() {
    ...
    if args.compare && !args.suite {
        #[cfg(feature = "board-pleco")]
        {
            let eval_str = if let Some(e) = args.eval.as_deref() {
                e
            } else if args.use_nnue {
                "nnue"
            } else {
            ...
            let nps_cozy = if dt_cozy > 0.0 {
                nodes_c as f64 / dt_cozy
            } else {
                0.0
            };
            // Pleco run
            // Pass depth as-is (0 means unlimited via ID), so Pleco honors movetime runs
            let (bm_p, sc_p, nodes_p, dt_p, depth_pleco) = run_pleco(
                &args.fen,
                args.threads,
                args.depth,
                if args.depth > 0 {
            ...
            let nps_p = if dt_p > 0.0 {
                nodes_p as f64 / dt_p
            } else {
                0.0
            };
            println!(
                "cozy:  bestmove={} score_cp={} nodes={} depth={} elapsed={:.3}s nps={:.1}",
                bm_c, sc_c, nodes_c, depth_cozy, dt_cozy, nps_cozy
            );
            println!(
                "pleco: bestmove={} score_cp={} nodes={} depth={} elapsed={:.3}s nps={:.1}",
                bm_p.unwrap_or_else(|| "(none)".to_string()),
                sc_p,
                nodes_p,
                depth_pleco,
                dt_p,
            ...
            if nps_cozy > nps_p {
                let diff = nps_cozy - nps_p;
                let pct = if nps_p > 0.0 {
                    diff / nps_p * 100.0
                } else {
                    0.0
                };
                println!("winner: cozy by +{:.1} nps (+{:.1}%)", diff, pct);
            } else if nps_p > nps_cozy {
                let diff = nps_p - nps_cozy;
                let pct = if nps_cozy > 0.0 {
                    diff / nps_cozy * 100.0
                } else {
                    0.0
                };
    ...
    if args.compare && args.suite {
        #[cfg(feature = "board-pleco")]
        {
                ...
                if let Ok(text) = std::fs::read_to_string(path) {
                    for (lineno, line) in text.lines().enumerate() {
                        ...
                        if l.starts_with('{') {
                            ...
                            let maybe_fen = l;
                            let fen_ok = if maybe_fen == "startpos" {
                                true
                            } else {
                                cozy_chess::Board::from_fen(maybe_fen, false).is_ok()
                            };
                            if fen_ok {
                    ...
                    eprintln!(
                        "warn: failed to read suite file {}; falling back to defaults",
                        path
                    );
                }
            }
            if suite.is_empty() {
                ...
                suite.push((
                    "Kiwipete".to_string(),
                    "r3k2r/p1ppqpb1/bn2pnp1/2pP4/1p2P3/P1N2N2/1P1BQPBP/R3K2R w KQkq - 0 1"
                        .to_string(),
                ));
            }
            ...
            for (i, (name, fen)) in suite.iter().enumerate() {
                ...
                if matches!(eval_mode, EvalMode::Nnue) {
                    ...
                    if let Some(q) = args.nnue_quant_file.as_deref() {
                        if let Ok(model) = piebot::eval::nnue::loader::QuantNnue::load_quantized(q)
                        {
                            sc.set_nnue_quant_model(model);
                        }
                    }
                    if let Some(d) = args.nnue_file.as_deref() {
                        if let Ok(nn) = piebot::eval::nnue::Nnue::load(d) {
                            sc.set_nnue_network(Some(nn));
                        }
                    }
                }
                let t0 = Instant::now();
                ...
                sum_nps_cozy += nps_c;
                sum_nps_pleco += nps_p;
                sum_depth_cozy += depth_c as u64;
                sum_depth_pleco += depth_p as u64;
            ...
            for (i, name, d_c, nps_c, d_p, nps_p) in &rows {
                println!(
                    "case={} name={} cozy_depth={} cozy_nps={:.1} pleco_depth={} pleco_nps={:.1}",
                    i, name, d_c, nps_c, d_p, nps_p
                );
            }
            ...
            if avg_nps_c > avg_nps_p {
                println!(
                    "winner_nps: cozy by +{:.1} nps (+{:.1}%)",
                    avg_nps_c - avg_nps_p,
                    if avg_nps_p > 0.0 {
                        (avg_nps_c - avg_nps_p) / avg_nps_p * 100.0
                    } else {
                        0.0
                    }
                );
            } else if avg_nps_p > avg_nps_c {
                println!(
                    "winner_nps: pleco by +{:.1} nps (+{:.1}%)",
                    avg_nps_p - avg_nps_c,
                    if avg_nps_c > 0.0 {
                        (avg_nps_p - avg_nps_c) / avg_nps_c * 100.0
                    } else {
                        0.0
                    }
                );
            }
            if avg_depth_c > avg_depth_p {
                println!("winner_depth: cozy by +{:.2}", avg_depth_c - avg_depth_p);
            } else if avg_depth_p > avg_depth_c {
                println!("winner_depth: pleco by +{:.2}", avg_depth_p - avg_depth_c);
            }
            return;
        }
        #[cfg(not(feature = "board-pleco"))]
    ...
    println!(
        "bestmove={} score_cp={} nodes={} elapsed={:.3}s nps={:.1}",
        bm.unwrap_or_else(|| "(none)".to_string()),
        sc,
        nodes,
        dt.as_secs_f64(),
        nps
