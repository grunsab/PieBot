...
fn mvv_lva_score(board: &Board, m: Move) -> i32 {
    let to = m.to;
    let from = m.from;
    let victim = piece_at(board, to)
        .map(|(_, p)| piece_value_cp(p))
        .unwrap_or(0);
    let attacker = piece_at(board, from)
        .map(|(_, p)| piece_value_cp(p))
        .unwrap_or(0);
    victim * 10 - attacker
}
...
impl Searcher {
    ...
    fn cont_index(parent_idx: usize, child_idx: usize) -> usize {
        let key = (parent_idx as u64).wrapping_mul(1_000_003)
            ^ (child_idx as u64).wrapping_mul(0x9E37_79B9);
        (key as usize) & ((1 << 18) - 1)
    }
        ...
        if self.order_captures || self.use_history || self.use_killers {
            ...
            if self.see_ordering && self.see_ordering_topk > 0 {
                for &m in moves.iter() {
                    ...
                    if (occ_mask & bit) != 0 {
                        cap_candidates.push((m, mvv_lva_score(board, m)));
                    }
                }
            ...
            moves.sort_by_key(|&m| {
                ...
                let hist = if self.use_history && remaining_depth >= self.hist_min_depth {
                ...
                let cont = if parent_move_idx != usize::MAX {
                    self.cont_hist[Self::cont_index(parent_move_idx, mi)]
                } else {
                    0
                };
                let see_b = if is_cap == 1 && self.see_ordering {
                    if self.see_ordering_topk == 0
                        || cap_candidates
                            .iter()
                            .take(self.see_ordering_topk.min(cap_candidates.len()))
                            .any(|&(mm, _)| mm == m)
                    {
    ...
    pub fn set_use_futility(&mut self, on: bool) {
        self.use_futility = on;
    }
    pub fn set_use_lmp(&mut self, on: bool) {
        self.use_lmp = on;
    }
    ...
    pub fn set_lmr_aggr(&mut self, v: i32) {
        self.lmr_aggr = v;
    }
    pub fn set_null_r_bonus(&mut self, v: i32) {
        self.null_r_bonus = v;
    }
    pub fn set_tt_first(&mut self, on: bool) {
        self.tt_first = on;
    }
    pub fn set_order_offset(&mut self, off: usize) {
        self.order_offset = off;
    }
    pub fn set_helper_tt_exact_only(&mut self, on: bool) {
        self.helper_tt_exact_only = on;
    }
    pub fn set_tail_policy(&mut self, p: TailPolicy) {
        self.tail_policy = p;
    }
    pub fn set_see_ordering(&mut self, on: bool) {
        self.see_ordering = on;
    }
    pub fn set_see_ordering_topk(&mut self, k: usize) {
        self.see_ordering_topk = k;
    }
    pub fn set_use_singular(&mut self, on: bool) {
        self.use_singular = on;
    }
    pub fn set_singular_margin(&mut self, cp: i32) {
        self.singular_margin_cp = cp;
    }
    pub fn set_iid_strong(&mut self, on: bool) {
        self.iid_strong = on;
    }
    pub fn set_smp_diversify(&mut self, on: bool) {
        self.smp_diversify = on;
    }
    pub fn set_smp_safe(&mut self, on: bool) {
        self.smp_safe = on;
    }
    ...
    pub fn search_movetime_lazy_smp(
        ...
        millis: u64,
        depth: u32,
    ) -> (Option<String>, i32, u64, u32) {
        let threads = self.threads.max(1);
        if threads == 1 {
            let (bm, sc, n) = self.search_movetime(board, millis, depth);
            return (bm, sc, n, self.last_depth);
        }
        ...
        let tail_policy = if smp_safe {
            TailPolicy::Full
        } else {
            self.tail_policy
        };
        let results: Vec<(usize, Option<String>, i32, u64, u32)> = (0..threads)
            .into_par_iter()
            .map(|wid| {
                ...
                if wid > 0 && smp_diversify {
                    w.set_lmr_aggr(1);
                    w.set_null_r_bonus(1);
                    w.set_tt_first(wid % 2 == 0);
                    w.set_order_offset(wid as usize);
                }
                ...
                if smp_safe {
                    w.set_use_futility(false);
                    w.set_use_lmp(false);
                } else {
                    w.set_use_futility(self.use_futility);
                    w.set_use_lmp(self.use_lmp);
                }
                // NNUE
                w.use_nnue = use_nnue;
        ...
        if let Some(maj) = majority {
            ...
            for r in &results {
                if r.1.as_deref() == Some(maj.as_str()) {
                    if cand.is_none() || r.4 > cand.as_ref().unwrap().4 {
                        cand = Some(r.clone());
                    }
                }
            }
            if let Some(c) = cand {
                ...
                for r in &results {
                    if r.4 > best.4 || (r.4 == best.4 && r.0 == 0 && best.0 != 0) {
                        best = r.clone();
                    }
                }
            }
    ...
    fn qsearch(&mut self, board: &Board, mut alpha: i32, beta: i32) -> i32 {
        // Terminal detection at horizon: stalemate or checkmate
        {
            ...
            if !has_legal {
                return self.eval_terminal(board, 0);
            }
        }
    ...
    fn search_depth_parallel_window(
        &mut self,
        board: &Board,
        depth: u32,
        alpha0: i32,
        ...
        if self.tt_first {
            if let Some(en) = self.tt_get(board) {
                if let Some(ttm) = en.best {
                    let trusted =
                        matches!(en.bound, Bound::Exact) || en.depth >= depth.saturating_sub(1);
                    if trusted {
                        if let Some(pos) = moves.iter().position(|&mv| mv == ttm) {
                            let mv = moves.remove(pos);
                            moves.insert(0, mv);
        ...
        if let Some(model) = &self.nnue_quant.as_ref().map(|qn| qn.model.clone()) {
            seed.nnue_quant = Some(QuantNetwork::new(model.clone()));
        }
        let mut best_score =
            -seed.alphabeta(&child, depth - 1, -beta0, -alpha0, 1, move_index(first));
        ...
        let tail_policy = if self.smp_safe {
            TailPolicy::Full
        } else {
            self.tail_policy
        };
        ...
        let results: Vec<(Move, i32, u64)> = tails
            .par_iter()
            .map(|&m| {
                let mut c = board.clone();
                c.play_unchecked(m);
                ...
                let mut sc = match tail_policy {
                    TailPolicy::Pvs => {
                        ...
                        if tsc > a {
                            tsc = -w.alphabeta(&c, next_depth, -beta0, -a, 1, move_index(m));
                        }
                        tsc
                    }
                    TailPolicy::Full => -w.alphabeta(&c, next_depth, -beta0, -a, 1, move_index(m)),
                };
                // Update shared alpha
                let mut cur = a;
                while sc > cur {
        ...
        SearchResult {
            bestmove: best_move_local.map(|mv| format!("{}", mv)),
            score_cp: best_score,
            nodes: self.nodes,
        }
    }
    ...
    fn alphabeta(
        ...
        if let Some(ref flag) = self.abort {
            if flag.load(Ordering::Relaxed) {
                return self.eval_cp_internal(board);
            }
        }
        self.nodes += 1;
        ...
        if self.use_nullmove && depth >= self.null_min_depth {
            ...
            if (board.checkers()).is_empty() {
                ...
                if null_ok {
                    ...
                    if self.null_r_bonus > 0 {
                        r = r.saturating_add(self.null_r_bonus as u32);
                    } else if self.null_r_bonus < 0 {
                        r = r.saturating_sub((-self.null_r_bonus) as u32).max(1);
                    }
                    if r >= depth {
                        r = depth - 1;
                    }
                    let score =
                        -self.alphabeta(&nb, depth - 1 - r, -beta, -beta + 1, ply + 1, usize::MAX);
                    if score >= beta {
        ...
        if moves.is_empty() {
            return self.eval_terminal(board, ply);
        }
        // TT move first
        if let Some(en) = self.tt_get(board) {
            if let Some(ttm) = en.best {
            ...
            let results: Vec<(Move, i32, u64)> = tails
                ...
                .map(|&m| {
                    ...
                    if abort_flag.load(Ordering::Relaxed) {
                        return (m, -MATE_SCORE, 0);
                    }
                    let score =
                        -w.alphabeta(&c, depth - 1, -MATE_SCORE, -a, ply + 1, move_index(m));
        ...
        let stand_eval = if self.use_futility && !self.smp_safe && depth <= 3 && !in_check_now {
            Some(self.eval_current(board))
        } else {
            None
        };
        ...
        if let Some(mv) = best_move_local {
            ...
            if self.use_history && bound == Bound::Lower && parent_move_idx != usize::MAX {
                ...
                if let Some(ch) = self.cont_hist.get_mut(idx) {
                    *ch = ch.saturating_add((depth as i32) * (depth as i32));
                }
            }
        }
        best
...
impl Searcher {
    ...
    fn tt_put(&mut self, board: &Board, depth: u32, score: i32, best: Option<Move>, bound: Bound) {
        ...
        if self.smp_safe {
            if !self.helper_tt_exact_only {
                if depth <= 3 {
                    if !matches!(bound, Bound::Exact) {
                        return;
                    }
                }
            }
        }
        let b = if self.helper_tt_exact_only {
            Bound::Exact
        } else {
            bound
        };
    ...
    fn search_depth_window(
        ...
        if self.threads > 1 && depth > 1 {
            return self.search_depth_parallel_window(board, depth, alpha, beta);
        }
        ...
        for m in moves.into_iter() {
            ...
            if score == crate::search::eval::DRAW_SCORE
                && crate::search::safety::is_stalemate(&child)
            {
                score -= 1;
            }
            if let Some(ch) = change {
    ...
    pub fn set_null_min_depth(&mut self, d: u32) {
        self.null_min_depth = d;
    }
    pub fn set_hist_min_depth(&mut self, d: u32) {
        self.hist_min_depth = d;
    }
    pub fn set_root_see_top_k(&mut self, k: usize) {
        self.root_see_top_k = k;
    }
            ...
            EvalMode::Nnue => {
                ...
                if self.use_nnue {
                    if let Some(qn) = &self.nnue_quant {
                        ...
                        return if board.side_to_move() == cozy_chess::Color::White {
                            score
                        } else {
                            -score
                        };
