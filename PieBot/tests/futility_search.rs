...
#[test]
fn futility_pruning_reduces_total_nodes_at_fixed_depth() {
    let depth = 8;
    ...
    // Skipping shallow quiet moves that cannot reach alpha should prune
    // substantially; tactical safety is gated by matein3 + A/B games.
    assert!(
        (exp_total as f64) < (base_total as f64) * 0.90,
        "futility should cut >=10% of nodes: experimental {exp_total} vs baseline {base_total}"
    );
}
