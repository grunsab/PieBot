...
#[test]
fn eval_blend_temp_quant_bias_only() {
    ...
    assert_eq!(
        q, 50,
        "quant bias-only NNUE at blend=100 should output 50 cp (temp search)"
    );

    s.set_eval_blend_percent(0);
    let q0 = s.qsearch_eval_cp(&b);
    assert_eq!(q0, pst, "blend=0 should equal PST eval (temp search)");

    s.set_eval_blend_percent(50);
    let q50 = s.qsearch_eval_cp(&b);
    let expected = (50 + pst) / 2;
    assert_eq!(
        q50, expected,
        "blend=50 should average NNUE and PST (temp search)"
    );
}
