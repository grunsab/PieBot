...
fn nullmove_reduces_nodes_midgame() {
    ...
    p1.use_aspiration = true;
    p1.aspiration_window_cp = 50;
    ...
    assert!(
        ratio <= 1.6,
        "nullmove should not explore substantially more nodes (ratio {:.2})",
        ratio
    );
}
...
fn nullmove_disabled_in_check() {
    ...
    let r2 = s2.search_with_params(&b, p2);
    assert_eq!(
        r2.score_cp, r1.score_cp,
        "nullmove in check should not change score"
    );
}
...
fn nullmove_reduces_nodes_shallow_depth() {
    ...
    let mut params_with_null = params;
    params_with_null.use_nullmove = true;
    ...
    assert!(
        ratio <= 1.1,
        "nullmove should not explore substantially more nodes even at depth 3 (ratio {:.2})",
        ratio
    );
    assert!(
        (result_with_null.score_cp - result_no_null.score_cp).abs() <= 100,
        "nullmove should not swing evaluation wildly at shallow depth"
    );
}
...
fn nullmove_disabled_in_simple_zugzwang() {
    ...
    assert_eq!(
        result_with_null.score_cp, result_no_null.score_cp,
        "nullmove must stay disabled in zugzwang to preserve score"
    );
    assert!(
        result_with_null.nodes >= result_no_null.nodes,
        "zugzwang guard should prevent pruning ({} vs {})",
        result_with_null.nodes,
        result_no_null.nodes
    );
}
