    ...
    fn alphabeta(
        ...
        for (idx, m) in moves.into_iter().enumerate() {
            ...
            if idx == 0 {
                ...
                // Log-log LMR: reductions grow with both depth and move
                // index instead of the flat 1-ply trim this replaces. Quiet,
                // non-checking, non-promotion moves only; never within two
                // plies of the horizon. A reduced fail-high is still verified
                // at full depth below before it can raise alpha.
                ...
                {
                    let schedule =
                        ((depth as f64).ln() * (idx as f64).ln() / 1.8) as u32;
                    schedule.max(1).min(depth.saturating_sub(2))
                } else {
                    0
                };
